...
export default {
  ...
  updated() {
    //this.url = "http://10.50.19.62:8080/file-management/fileupload/111/1/2";
    this.url = this.$http.adornUrl(
      `/fileupload/${this.dataForm.id}/${this.dataForm.imgType}`
    );
  ...
  methods: {
    ...
    init(id) {
      this.addTypeList = this.options.concat();
      console.log(JSON.stringify(this.addTypeList) + "所有的类型");
      this.getPicList(id);
    ...
    getPicList(id) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          console.log(this.typeList + "已有的类型");
          var item = "";
          ...
          this.typeList = [];
          console.log(JSON.stringify(this.addTypeList) + "需要补录的类型");
        }
