<template>
  <div class="mod-user">
    ...
    <file-out-sure ref="fileOutSure"></file-out-sure>
    <!--图片上传组件-->
    <file-pic-input ref="filePicInput"></file-pic-input>
  </div>
</template>

<script>
import FileOutSure from "./fileOut-sure.vue";
import FilePicInput from "./filePicInput.vue";
export default {
  data() {
    return {
      dataForm: {
  ...
  components: { FileOutSure, FilePicInput },
  // 这里实现了页面刷新，自动执行函数
  activated() {
    this.getDataList();
  },
  methods: {
    ...
    die(id) {
      this.$refs.fileOutSure.onOpen();
      this.$nextTick(() => {
        this.$refs.fileOutSure.init(id);
      });
    },
    //图片资料上传
    picAdd(id) {
      this.$refs.filePicInput.onOpen();
      this.$nextTick(() => {
        this.$refs.filePicInput.init(id);
      });
    },
  },
};
