<template>
  <div class="mod-user">
    ...
    <fired-sure ref="firedSure"></fired-sure>
    <!--图片上传组件-->
    <fired-pic-input ref="firedPicInput"></fired-pic-input>
  </div>
</template>

<script>
import FiredSure from "./fired-sure.vue";
import FiredPicInput from "./firedPicInput.vue";
export default {
  data() {
    return {
      dataForm: {
  ...
  components: { FiredSure, FiredPicInput },
  // 这里实现了页面刷新，自动执行函数
  activated() {
    this.getDataList();
  },
  methods: {
    ...
    die(id) {
      this.$refs.firedSure.onOpen();
      this.$nextTick(() => {
        this.$refs.firedSure.init(id);
      });
    },
    //图片资料上传
    picAdd(id) {
      this.$refs.firedPicInput.onOpen();
      this.$nextTick(() => {
        this.$refs.firedPicInput.init(id);
      });
    },
  },
};
