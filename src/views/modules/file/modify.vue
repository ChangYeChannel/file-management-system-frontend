<template>
  <div class="mod-user">
    ...
    <!--这个组件不涉及图片上传-->
    <modify-sure ref="modifySure" @refreshDataList="getDataList"></modify-sure>
    <!--图片上传组件-->
    <modify-pic-input ref="modifyPicInput"></modify-pic-input>
  </div>
</template>

<script>
import ModifySure from "./modify-sure.vue";
import ModifyPicInput from "./modifyPicInput.vue";
export default {
  data() {
    return {
      dataForm: {
  ...
  methods: {
    ...
    //图片资料上传
    picAdd(id) {
      this.$refs.modifyPicInput.onOpen();
      this.$nextTick(() => {
        this.$refs.modifyPicInput.init(id);
      });
    },
    //档案补录
    fileAdd(id) {
      this.$refs.modifySure.onOpen();
      this.$nextTick(() => {
        this.$refs.modifySure.init(id);
      });
    },
    // 删除
    deleteHandle(id) {
